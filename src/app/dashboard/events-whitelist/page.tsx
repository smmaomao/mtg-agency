  ...
  return (
    <div>
      <div className="mb-6 flex items-end justify-between border-b border-gray-200 pb-4">
        <div>
          <h1 className="text-xl font-semibold text-black tracking-tight">Events 白名单</h1>
          <p className="mt-1 text-sm text-gray-400">管理回调接口的访问 Token</p>
        </div>
      ...
      {/* Modal */}
      <Modal open={showModal} onClose={() => setShowModal(false)} title={editing ? '编辑白名单' : '新增白名单'} maxWidth={448}>
        <form onSubmit={handleSubmit} className="space-y-4 p-5">
          <Field label="名称">
            <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
              ...
              placeholder="例如：Mintegral、AppLovin" required />
          </Field>
          <Field label="备注">
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={() => setShowModal(false)} className="focus-ring flex-1 rounded-full border border-gray-300 py-2 text-[13px] text-gray-600 transition-colors hover:bg-gray-50">取消</button>
            <button type="submit" className="focus-ring flex-1 rounded-full bg-blue-600 py-2 text-[13px] font-medium text-white transition-colors hover:bg-blue-700">{editing ? '保存' : '创建'}</button>
          </div>
        </form>
      </Modal>
    </div>
