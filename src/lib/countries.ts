// 国家/地区码对照表（ISO 3166-1 alpha-2，共 249 个正式代码）。前端维护，供下拉选择使用。
// 下拉里展示 "CODE-名称"，但提交给后端只发 CODE。
...
const CODE_MAP = new Map<string, Country>(COUNTRIES.map(c => [c.code, c]))

// 单个国家码 -> "ID-印尼"（找不到时回退为原码，兼容历史数据）
export function countryLabel(code: string): string {
  const c = CODE_MAP.get(code)
  return c ? `${c.code}-${c.name}` : code
}

// 国家码数组 -> "ID-印尼, JP-日本"（空时返回占位符）
export function countryLabels(codes: string[] | null | undefined, empty = '-'): string {
